...
export default function RoleList() {
    ...
    const handleOk = () => {
        setIsModalVisible(false);//模态框不可见
        // 同步dataSource
        // currentID和currentRights是点击编辑按钮时候保存下来的
    ...
    return (
        <div>
            {/* rowKey:表格行 key 的取值，可以是字符串或一个函数
                pagination 不写默认开始，在右下角
            
            */}
            <Table dataSource={dataSource} columns={columns} rowKey={(item) => item.id} pagination={{ position: ['bottomCenter'] }} />
            {/* 
               visible ：对话框是否可见
               通过modal内嵌tree结构实现编辑按钮的响应布局
            */}
            <Modal title={t("role.permission")} visible={isModalVisible} onOk={handleOk} onCancel={handleCancel} okText={t("common.ok")}
                cancelText={t("common.cancel")}>
                {/*  
